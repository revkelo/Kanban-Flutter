...
/* ============================================================
   CONSTANTES LEGALES / PRIVACIDAD (Huawei regla 7.1 / 7.5)
   IMPORTANTE:
   - kAppName debe ser EXACTAMENTE el nombre que subiste en AppGallery.
   - kDeveloperName debe ser EXACTAMENTE el nombre del desarrollador
     que aparece en tu cuenta de AppGallery Connect.
   - kContactEmail es tu email de contacto de privacidad.
   ============================================================ */
const String kAppName = 'KanbanAPP'; // <-- reemplaza con el nombre en AppGallery
const String kDeveloperName = 'Kevin Gonzalez Revkelo'; // <-- nombre desarrollador legal
const String kContactEmail = 'kgagudelo@gmail.com';
const String kPrivacyAcceptedKey = 'privacyAccepted_v1';
...
/// RootGate decide si mostrar:
/// - la pantalla de aceptación de privacidad (primer arranque)
/// - o el tablero Kanban normal
...
class _RootGateState extends State<RootGate> {
  ...
  Future<void> _initPrefs() async {
    ...
    final a = sp.getBool(kPrivacyAcceptedKey) ?? false;
    setState(() {
      _accepted = a;
      _loading = false;
    });
  ...
  Future<void> _acceptNow() async {
    final sp = await SharedPreferences.getInstance();
    await sp.setBool(kPrivacyAcceptedKey, true);
    setState(() {
      _accepted = true;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!_accepted) {
      // Primera vez: bloquear uso hasta aceptar
      return PrivacyGateScreen(
        onAccepted: _acceptNow,
      );
    }

    return const KanbanHome();
  }
}

/* ===================== PANTALLA DE PRIVACIDAD INICIAL ===================== */

class PrivacyGateScreen extends StatelessWidget {
  final Future<void> Function() onAccepted;
  const PrivacyGateScreen({super.key, required this.onAccepted});
        ...
        child: Padding(
          ...
          child: ListView(
            children: [
              ...
              Text(
                'Para continuar usando $kAppName necesitamos que leas y aceptes '
                    'nuestra Política de Privacidad. '
                    '$kAppName es desarrollada y operada por $kDeveloperName. '
                    'Recogemos datos básicos necesarios para que la aplicación funcione '
                    '(por ejemplo: tareas que creas, fecha de creación, opcionalmente fecha de vencimiento) '
                    'y almacenamos esa información localmente en tu dispositivo. '
              ...
              Text(
                'Al continuar declaras que:\n'
                    ...
                    '• Aceptas el tratamiento de los datos descritos.\n'
                    '• Eres mayor de edad según la ley local o tienes autorización.',
                style: Theme.of(context).textTheme.bodyMedium,
              ),
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: FilledButton.icon(
            ...
            onPressed: () async {
              await onAccepted();
              // al aceptar, simplemente se hace setState en RootGate y
              // RootGate reconstruye mostrando KanbanHome
            },
          ),
        ),
      ),
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: ListView(
            children: [
              Text(
                'Política de Privacidad – $kAppName',
              ...
              Text(
                '$kAppName es desarrollada y operada por $kDeveloperName '
                    '("nosotros", "nuestro"). Esta Política explica cómo '
                    'recogemos, usamos y protegemos tu información.',
              ...
              Text(
                '- Datos de uso dentro de la app: las tareas que creas, '
                    'sus títulos, descripciones, estados de progreso y fechas.\n'
                    '- Datos técnicos básicos del dispositivo (versión del sistema, '
                    'idioma) para mejorar estabilidad.',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              Text(
                'Usamos la información para:\n'
                    '- Mostrar y sincronizar tus tarjetas dentro del tablero Kanban.\n'
                    '- Mantener la funcionalidad central de la app.\n'
                    '- Mejorar calidad y corregir errores.',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              Text(
                'Los datos de tus tareas se guardan localmente en tu dispositivo '
                    'mediante almacenamiento interno. No vendemos tu información.',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              Text(
                'No compartimos información personal con terceros, '
                    'excepto cuando sea requerido por ley o para garantizar '
                    'la seguridad técnica y el cumplimiento legal.',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              Text(
                'En cualquier momento puedes solicitar acceso, corrección o '
                    'eliminación de tu información. Para ello, escríbenos a '
                    '$kContactEmail indicando el nombre de la app ($kAppName).',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              Text(
                '$kAppName no está dirigida a menores sin el debido '
              ...
              Text(
                'Si tienes dudas sobre esta Política de Privacidad '
                    'o sobre el manejo de datos, puedes contactarnos en '
                    '$kContactEmail.',
                style: textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
...
class _KanbanHomeState extends State<KanbanHome>
  ...
  Future<void> _dialogoNuevaTarea(
      [Estado estadoInicial = Estado.backlog]) async {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Scaffold(
          appBar: AppBar(
            // Antes estaba en 0 para ocultar totalmente la AppBar.
            // Ahora dejamos altura mínima para que se vea más "app completa"
            // y porque necesitamos un botón para ver la privacidad
            toolbarHeight: 64,
            title: Text(
              kAppName,
                    ...
                    child: Row(
                      children: [
                        ...
                        PopupMenuButton<SortMode>(
                          ...
                          itemBuilder: (ctx) => [
                            _itemSort(ctx, SortMode.creacionAsc,
                                'Creación ↑'),
                            _itemSort(ctx, SortMode.creacionDesc,
                                'Creación ↓'),
                            _itemSort(ctx, SortMode.venceAsc,
                                'Vencimiento ↑'),
                            _itemSort(ctx, SortMode.venceDesc,
                                'Vencimiento ↓'),
                            _itemSort(ctx, SortMode.titulo,
                                'Título A→Z'),
                          ],
                          child: Row(
                            children: [
          ...
          floatingActionButton: esMovil
            ...
            onPressed: () => _dialogoNuevaTarea(
                _estadoDeIndex(_tabController.index)),
  ...
  PopupMenuItem<SortMode> _itemSort(
      BuildContext ctx, SortMode m, String label) {
...
class _CardTarea extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final vencida =
        t.vence != null && t.vence!.isBefore(DateTime.now());
    return Card(
                    ...
                    Wrap(
                      ...
                      children: [
                        _ChipInfo(
                          icon: Icons.today,
                          label:
                          'Creada ${_fmtFechaCorta(t.creada)}',
                        ),
                        _ChipInfo(
                          icon: Icons.event,
                          label: t.vence == null
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      actions: [
        TextButton(
          onPressed: () =>
              Navigator.pop<Tarea?>(context, null),
